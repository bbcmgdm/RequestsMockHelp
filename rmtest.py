...
class TestAPIClient(object):
    def test_qs(self):
        ...
        adapter.register_uri('GET', 'https://test.api.com/things', text='{"foo": "bar"}')

        api = APIClient('https://test.api.com', session=session)
        response = api.get('things', foo='bar', bar='baz')

        assert response == {'foo': 'bar'}
        assert len(adapter.request_history) == 1
        # assert adapter.request_history[0].url == 'https://test.api.com/things'
    ...
    def test_qs_in_url(self):
        adapter, session = get_session()
        adapter.register_uri('GET', 'https://test.api.com/things?foo=bar&bar=baz', text='{"foo": "bar"}')
